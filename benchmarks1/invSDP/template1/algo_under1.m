function algo_under1(benchmark, adeg)
...
if diagnostics.problem == 0
    ...
    filename=strcat("results/",benchmark,".txt");
    fileID = fopen(filename, 'a');
    % fileid = fopen('output.txt','a');
    aftertime=toc;
    fprintf(fileID,'after%s seconds\n',aftertime);
    fprintf(fileID,'benchmark:%s with adeg=%d, a solution is: %s\n',benchmark,adeg,s);
    fclose(fileID);
    %display(a_range);
    %{
    ...
    fileID = fopen(filename,'a');
    fprintf(fileID,'after%s seconds\n',aftertime);
    fprintf(fileID,'benchmark:%s with adeg=%d, no solution\n',benchmark,adeg);
    fclose(fileID);
    fprintf('adeg=%d\n',adeg); 
